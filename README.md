### Hi there 👋

I'm Rahaman Leon — a developer building AI tools, open-source APIs, and automation systems.

## 🔭 What I'm working on

- **PicoClaw** 🦞 — A tiny, fast Go AI assistant that automates the mundane
- **FreeLLMAPI** — One API endpoint for 12+ free LLM providers
- **Shazam-API** 🎵 — Production-ready audio recognition API
- **World Cup 2026 API** 🏆 — Free REST API for the biggest sporting event
- **Dora AI Agent** 🤖 — Lightweight AI agent with persistent memory

## 📊 GitHub Stats

![GitHub stats](https://github-readme-stats.vercel.app/api?username=rahamanleon&show_icons=true&theme=tokyonight)

## 🚀 Featured Projects

| Project | Description | Stars |
|---------|-------------|-------|
| [FreeLLMAPI](https://github.com/rahamanleon/freellmapi) | Free LLM API proxy - one endpoint for 12+ providers | ⭐ |
| [Shazam-API](https://github.com/rahamanleon/Shazam-API) | Audio recognition via acoustic fingerprinting | ⭐ |
| [WorldCup2026](https://github.com/rahamanleon/worldcup2026) | FIFA World Cup 2026 REST API | |
| [Dora Agent](https://github.com/rahamanleon/Dora-agent) | AI Agent with memory and web tools | |
| [PicoClaw](https://github.com/rahamanleon/picoclaw) | Tiny Go AI assistant | |


---

## 📬 Contact

- **Email**: rahamanleon16@gmail.com
- **Repository**: [https://github.com/rahamanleon/rahamanleon](https://github.com/rahamanleon/rahamanleon)
- **Issues**: [https://github.com/rahamanleon/rahamanleon/issues](https://github.com/rahamanleon/rahamanleon/issues)
- **GitHub**: [https://github.com/rahamanleon](https://github.com/rahamanleon)
